jasmine.NewReporter = function(options) {
  ...
  this.jasmineDone = function() {
    ...
    var checkbox = doc.body.getElementsByTagName("input")[0];

    checkbox.checked = !env.catchingExceptions();
    checkbox.onclick = function() { setQueryParam("catch", !checkbox.checked); };
  ...
  function buildSpecFilter() {
    var specFilterParam = queryStringToObject().spec || "";

    specFilterPattern = new RegExp(specFilterParam);
  }

  function queryStringToObject() {
    var paramStr = win.location.search.substring(1),
      params = [], paramMap = {};
  ...
  function setQueryParam(key, value) {
    var paramMap = queryStringToObject();
    paramMap[key] = value;
    win.location.search = toQueryString(paramMap);
  }
